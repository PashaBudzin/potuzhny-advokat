...
export function extractTags(arrayBuffer: ArrayBuffer) {
    try {
        ...
        if (allTags.size === 0) {
            console.warn("No tags found in document. Checked document.xml, headers, and footers.");
        }
...
export function generateDocx(arrayBuffer: ArrayBuffer, data: Record<string, string>) {
    const zip = new PizZip(arrayBuffer);
