        ...
        <>
            ...
            <CardFooter className="pb-4">
                <div className="flex justify-end w-full">
                    <Button onClick={() => setCurrentStep(2)} disabled={files.length == 0}>
                        Наступний крок
                    </Button>
                </div>
            </CardFooter>
...
function DataPreview({ state }: { state: "none" | "loading" | "success" | "failed" }) {
    const [pozovData, setPozovData] = useAtom(pozovDataAtom);
...
function ThirdStep() {
    ...
    const saveDocument = useCallback(() => {
        if (docxFile) {
            saveAs(docxFile, `позов ${pozovData?.data?.["ПІБ позивача"] ?? "pozov"}.docx`);
        }
    }, [docxFile, pozovData]);
